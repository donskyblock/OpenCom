...
async function assertExpectedDatabase(expectedDatabase: string) {
  ...
  if (currentDatabase !== expectedDatabase) {
    throw new Error(
      `Refusing to run server-node migrations against ${currentDatabase ?? "<none>"}; expected ${expectedDatabase}. Check NODE_DATABASE_URL.`,
    );
  }
}
...
async function main() {
  await assertExpectedDatabase("ods_node");
